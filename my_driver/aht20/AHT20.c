...
static bool AHT20_write(uint8_t data[], uint32_t len)
{
	...
	IIC2_CHECK_EVENT(I2C_EVENT_MASTER_MODE_SELECT, 1000);
	I2C_Send7bitAddress(I2C2, 0x70, I2C_Direction_Transmitter);
	IIC2_CHECK_EVENT(I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED, 1000);
	for (uint32_t i = 0; i < len; i++)
	{
		I2C_SendData(I2C2, data[i]);
		IIC2_CHECK_EVENT(I2C_EVENT_MASTER_BYTE_TRANSMITTING, 1000);
	}
...
static bool AHT20_read(uint8_t data[], uint32_t len)
{
	...
	IIC2_CHECK_EVENT(I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED, 1000);
	for (uint32_t i = 0; i < len; i++)
	{
		if (i == len - 1)
			I2C_AcknowledgeConfig(I2C2, DISABLE);
		IIC2_CHECK_EVENT(I2C_EVENT_MASTER_BYTE_RECEIVED, 1000);
